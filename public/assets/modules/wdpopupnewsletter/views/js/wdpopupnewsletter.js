...
{
	$(document).on('click', '.open .wd-coupon-small, .open .share-coupon-small-wrapper', function()
	{
		showDialog();
	});
	$(document).on('click', '.close .wd-coupon-small, .close .share-coupon-small-wrapper', function()
	{
...
{  
	var data={'task':'showPopup'};
		...
		type: "POST",
		cache: false,
		url: wd_coupon_url + '/front-end-ajax.php',
		dataType : "json",
		data: data,
		complete: function(){},
	...
	$("#overlay").show();
    $(".wd-popup").show();
	$('.wd-show-popup').removeClass('open').addClass('close');
}
...
{  
	...
	$("#overlay").hide();
    $(".wd-popup").hide();
	$('.wd-show-popup').removeClass('close').addClass('open');	
}
